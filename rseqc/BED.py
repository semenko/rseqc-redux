...
class CompareBED:
    ...
    def distribBedWithStrand(self, outfile=None, output=True):
        ...
        for line in self.A_fh:
            ...
            if len(fields) < 6:
                print(
                    "[NOTE:input bed must be at least 6 columns] skipped this line: " + line, end=" ", file=sys.stderr
                )
            if fields[5] == "-":
                chrom = fields[0].upper()
                mid = int(fields[1]) + int((int(fields[2]) - int(fields[1])) / 2)
        ...
        for line in self.B_fh:
            ...
            if chrom in ranges:
                if mod == 0:  # TSS-up, TSS-down
                    ...
                    if strand == "+":
                        hits = ranges[chrom].find(TSS - upStream, TES + downStream)
                    if strand == "-":
                        hits = ranges[chrom].find(TSS + upStream, TES - downStream)
                    ...
                    if strand == "+":
                        hits = ranges[chrom].find(CDSS - upStream, CDSE + downStream)
                    if strand == "-":
                        hits = ranges[chrom].find(CDSS + upStream, CDSE - downStream)
