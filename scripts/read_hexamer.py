...
def main():
    ...
    if options.ref_genome and file_exist(options.ref_genome):
        ...
        read_table[os.path.basename(options.ref_genome)] = FrameKmer.kmer_freq_file(
            fastafile=options.ref_genome, word_size=6, step_size=1, frame=0
        )
        read_file_names.append(os.path.basename(options.ref_genome))
        read_file_sum[os.path.basename(options.ref_genome)] = float(
            sum(read_table[os.path.basename(options.ref_genome)].values())
        )
        print("Done.", file=sys.stderr)

    if options.ref_gene and file_exist(options.ref_gene):
        print("Calculate hexamer of " + options.ref_gene + " file ...", end=" ", file=sys.stderr)
        read_table[os.path.basename(options.ref_gene)] = FrameKmer.kmer_freq_file(
            fastafile=options.ref_gene, word_size=6, step_size=1, frame=0
        )
        read_file_names.append(os.path.basename(options.ref_gene))
        read_file_sum[os.path.basename(options.ref_gene)] = float(
            sum(read_table[os.path.basename(options.ref_gene)].values())
        )
        print("Done.", file=sys.stderr)
